...
public record Task
{
    ...
    internal sealed class JsonConverter : JsonConverter<Task>
    {
        ...
        )
        {
            ...
            {
                "classification" =>
                    jsonWithoutDiscriminator.Deserialize<Hume.ExpressionMeasurement.Batch.TaskClassification?>(
                        options
                    )
                        ?? throw new JsonException(
                ...
                "regression" =>
                    jsonWithoutDiscriminator.Deserialize<Hume.ExpressionMeasurement.Batch.TaskRegression?>(
                        options
                    )
                        ?? throw new JsonException(
